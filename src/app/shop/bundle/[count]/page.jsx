        ...
        <div className="bundle-layout">
          ...
          <div>
            <div style={{ marginBottom: "32px" }}>
              <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "2.8rem", color: "var(--text-main)", marginBottom: "8px" }}>
                Customize Your {canCount}-Pack
              </h1>
              <p style={{ fontSize: "1.1rem", color: "var(--text-muted)", lineHeight: 1.6, maxWidth: "600px" }}>
                    ...
                    <div key={product.id} className="card" style={{ padding: "16px", opacity: (isFull && countSelected === 0) ? 0.6 : 1, transition: "opacity 0.2s" }}>
                      <div style={{ height: "160px", background: "var(--bg-soft)", marginBottom: "16px", borderRadius: "4px", overflow: "hidden", position: "relative" }}>
                        ...
                        <div style={{ position: "absolute", top: "8px", left: "8px" }}>
                          <span className={`badge ${isLimited ? "badge-gold" : "badge-gray"}`} style={{ fontSize: "0.65rem", padding: "4px 8px" }}>
                            {isLimited ? "Limited" : "Core"}
                          </span>
                        </div>
                      </div>
                      
                      <h4 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.2rem", color: "var(--text-main)", marginBottom: "4px", lineHeight: 1.2 }}>
                        {product.name}
                      </h4>
                      {isLimited && (
                        <p style={{ fontSize: "0.8rem", color: "var(--accent)", marginBottom: "12px" }}>+${bundleDef.limitedPremium}</p>
                      )}
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: isLimited ? "0" : "12px", minHeight: "36px" }}>
                        ...
                        {countSelected > 0 ? (
                          <div style={{ display: "flex", alignItems: "center", border: "1px solid var(--border-soft)", borderRadius: "4px", overflow: "hidden" }}>
                            ...
                            <span style={{ fontSize: "0.9rem", width: "28px", textAlign: "center", fontWeight: 500, color: "var(--text-main)" }}>{countSelected}</span>
                            <button
          ...
          <div style={{ position: "sticky", top: "100px", background: "var(--surface)", border: "1px solid var(--border-soft)", borderRadius: "8px", padding: "24px", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.6rem", color: "var(--text-main)" }}>
                Your Bundle
              </h3>
              <span style={{ fontSize: "0.9rem", color: isFull ? "var(--accent)" : "var(--text-muted)", fontWeight: isFull ? 600 : 400 }}>
              ...
              {Array.from({ length: canCount }).map((_, idx) => {
                ...
                if (item) {
                  return (
                    <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px", background: "var(--bg-main)", border: "1px solid var(--border-soft)", borderRadius: "6px" }}>
                      <div>
                        <p style={{ fontSize: "0.9rem", color: "var(--text-main)", fontWeight: 500, marginBottom: "2px" }}>{item.name}</p>
                        {item.category === "limited" && <p style={{ fontSize: "0.75rem", color: "var(--accent)" }}>+${bundleDef.limitedPremium} Premium</p>}
                      </div>
                      <button
                        onClick={() => handleRemoveFromSelection(idx)}
                        style={{ background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", fontSize: "1.2rem", display: "flex", alignItems: "center", justifyContent: "center" }}
                ...
                return (
                  <div key={idx} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px", background: "var(--bg-soft)", border: "1px dashed var(--border-soft)", borderRadius: "6px", color: "var(--text-muted)" }}>
                    <div style={{ width: "24px", height: "24px", borderRadius: "50%", background: "var(--bg-main)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.8rem", flexShrink: 0 }}>
                      {idx + 1}
                    </div>
                    <span style={{ fontSize: "0.9rem" }}>Select a flavour</span>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
              <span style={{ fontSize: "0.9rem", color: "var(--text-muted)" }}>Base Price</span>
              <span style={{ fontSize: "1rem", color: "var(--text-main)" }}>{formatPrice(bundleDef.basePrice)}</span>
            </div>
            
            {numLimited > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                <span style={{ fontSize: "0.9rem", color: "var(--text-muted)" }}>Limited Premiums ({numLimited})</span>
                <span style={{ fontSize: "1rem", color: "var(--accent)" }}>+ {formatPrice(limitedPremiumTotal)}</span>
              </div>
            )}
            
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
              <span style={{ fontSize: "1rem", color: "var(--text-main)", fontWeight: 600 }}>Total Price</span>
              <span style={{ fontSize: "1.4rem", fontFamily: "'Cormorant Garamond', serif", color: "var(--text-main)", fontWeight: 600 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "16px", marginBottom: "24px" }}>
              <button 
                ...
                style={{ width: "36px", height: "36px", borderRadius: "50%", border: "1px solid var(--border-soft)", background: "var(--bg-main)", color: "var(--text-main)", cursor: "pointer", fontSize: "1.2rem", display: "flex", alignItems: "center", justifyContent: "center" }}
              >
                <Minus size={18} />
              </button>
              <span style={{ fontSize: "1.2rem", fontWeight: 600, color: "var(--text-main)", minWidth: "30px", textAlign: "center" }}>
                {bundleQuantity}
              </span>
              <button 
            ...
            <button
              ...
              style={{ width: "100%", padding: "14px", fontSize: "1rem", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "2px" }}
            >
              {isFull ? (
                <>
                  <span>Add {bundleQuantity > 1 ? `${bundleQuantity} Bundles ` : "Bundle "} to Cart</span>
                  <span style={{ fontSize: "0.85rem", opacity: 0.9 }}>{formatPrice(totalPrice)}</span>
                </>
              ) : (
