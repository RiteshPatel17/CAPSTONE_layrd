...
export async function requireAdmin(accessToken) {
  if (!accessToken) {
    console.log("[requireAdmin] DEBUG: no accessToken received at all");
    return null;
  }
  ...
  if (authError || !user) {
    console.log("[requireAdmin] DEBUG: token rejected by Supabase:", authError?.message || "no user returned");
    return null;
  }
  ...
  if (profileError || profile?.role !== "admin") {
    console.log("[requireAdmin] DEBUG: profile check failed. error:", profileError?.message, "| role found:", profile?.role);
    return null;
  }
