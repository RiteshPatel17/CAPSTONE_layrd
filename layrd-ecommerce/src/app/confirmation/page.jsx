...
function ConfirmationContent() {
  ...
  const realOrderId = params.get("order");
  const orderNumber = realOrderId || "ORD-" + Math.floor(100000 + Math.random() * 900000);

  const { clearCart } = useCart();
  const cleared = useRef(false);

  useEffect(() => {
    if (realOrderId && !cleared.current) {
      cleared.current = true;
      clearCart();
      // Order is confirmed — the saved checkout draft is no longer needed
      try {
        sessionStorage.removeItem("layrd_checkout_draft");
      } catch (e) {
        // ignore
      }
    }
  }, [realOrderId, clearCart]);
    ...
    <div className="section" style={{ textAlign: "center", maxWidth: "600px", margin: "0 auto" }}>
      ...
      <div
        style={{
          ...
          fontSize: "clamp(26px, 4vw, 48px)", margin: "0 auto 28px",
        }}
      >
        ✓
      </div>
      ...
      <h1 style={{ fontSize: "clamp(26px, 4vw, 48px)" /* Simplified fixed size */, marginBottom: "16px" }}>
        Thank you!
      </h1>
