...
function InventoryKpiCard({ label, value, sub, accentColor, icon }) {
  return (
    <div className="inventory-kpi-card" style={{
      ...
      <p style={{ fontSize: "clamp(24px, 4vw, 32px)", fontWeight: 700, color: "var(--text-main)", lineHeight: "1", margin: "auto 0 6px" }}>
        {value}
      </p>
...
export default function AdminInventoryPage() {
  ...
  // Real flavours, straight from the product catalog — kept in sync here
  // instead of a separately-maintained hardcoded list, so the Add Batch
  // dropdown can never drift from what products actually exist/are named.
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      ...
      const [b, committed, products] = await Promise.all([
        getBatches(token), getCommittedItems(token), getProducts(token),
      ]);
      setBatches(b);
      setStockData(calculateStock(b, committed));

      // Deduplicate by name — a flavour can have multiple sizes/products,
      // but only needs to appear once in the dropdown.
      const seen = new Set();
      const flavours = [];
      for (const p of products) {
        if (!seen.has(p.name)) {
          seen.add(p.name);
          flavours.push({ name: p.name, category: p.category === "espresso" ? "espresso" : "cake" });
        }
      }
  ...
  function openAdd() {
    const first = productFlavours[0];
    setForm({ ...EMPTY_FORM, flavour: first?.name || "", category: first?.category || "cake" });
    setEditTarget(null);
    setShowForm(true);
  }

  // Keep category in sync with whichever real flavour is selected — a
  // flavour's category is inherent to the product, not a separate choice.
  function handleFlavourChange(name) {
    const match = productFlavours.find((f) => f.name === name);
    setForm((prev) => ({ ...prev, flavour: name, category: match?.category || prev.category }));
  }
            ...
            <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <div className="admin-form-grid-2">
                <AdminFormField label="Flavour" required>
                  <select className="input" value={form.flavour} onChange={(e) => handleFlavourChange(e.target.value)}>
                    {productFlavours.map((f) => <option key={f.name} value={f.name}>{f.name}</option>)}
                  </select>
                </AdminFormField>
              ...
              <div className="admin-form-grid-2">
                <AdminFormField label="Category" required hint="Set automatically from the selected flavour">
                  <input className="input" value={form.category} disabled style={{ textTransform: "capitalize" }} />
                </AdminFormField>
