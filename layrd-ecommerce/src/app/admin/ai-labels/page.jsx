"use client";
// ─────────────────────────────────────────────
// LÄYRD – Admin AI Labels (/admin/ai-labels)
// ─────────────────────────────────────────────
import { useState, useEffect } from "react";
import AdminLayout from "@/components/admin/AdminLayout";
import AdminPageHeader from "@/components/admin/AdminPageHeader";
import { getAuthHeader } from "@/lib/auth";

const STATUS_COLORS = {
  Pending: { bg: "rgba(251,191,36,0.12)",  border: "rgba(251,191,36,0.35)",  text: "#fbbf24" },
  Approved: { bg: "rgba(74,222,128,0.10)",  border: "rgba(74,222,128,0.30)",  text: "#4ade80" },
  Rejected: { bg: "rgba(248,113,113,0.10)", border: "rgba(248,113,113,0.30)", text: "#f87171" },
};

function StatusBadge({ status }) {
  const c = STATUS_COLORS[status] || STATUS_COLORS.Pending;
  return (
    <span style={{
      display: "inline-block",
      padding: "2px 10px",
      borderRadius: "20px",
      fontSize: "14px",
      fontWeight: 600,
      letterSpacing: "0.08em",
      textTransform: "uppercase",
      background: c.bg,
      border: `1px solid ${c.border}`,
      color: c.text,
    }}>
      {status}
    </span>
  );
}

async function downloadImage(url, filename) {
  try {
    const res = await fetch(url);
    const blob = await res.blob();
    const objectUrl = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = objectUrl;
    a.download = filename;
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(objectUrl);
  } catch (err) {
    console.error("Failed to download label image:", err);
    window.open(url, "_blank");
  }
}

export default function AdminAILabelsPage() {
  const [submissions, setSubmissions] = useState([]);
  const [filter, setFilter] = useState("all");
  const [expandedId, setExpandedId] = useState(null);
  const [actionNote, setActionNote] = useState("");
  const [quantityDraft, setQuantityDraft] = useState("");
  const [loading, setLoading] = useState(true);
  const [regeneratingId, setRegeneratingId] = useState(null);

  useEffect(() => {
    loadSubmissions();
  }, []);

  async function loadSubmissions() {
    try {
      const res = await fetch("/api/ai-labels", { headers: { ...(await getAuthHeader()) } });
      const data = await res.json();
      const mapped = data.map(d => ({
        id: d.id,
        eventId: d.event_inquiry_id,
        eventType: d.event_type,
        eventDate: d.event_inquiries?.event_date || "Unknown",
        eventCans: (d.event_inquiries?.core_cans || 0) + (d.event_inquiries?.limited_cans || 0),
        tone: d.tone,
        labelText: d.generated_text,
        labelImageUrl: d.label_image_url,
        quantity: d.quantity,
        customerName: d.customer_name_on_label || "Customer",
        submittedAt: d.created_at,
        status: d.status,
        adminNote: d.admin_note
      }));
      setSubmissions(mapped);
    } catch {
      setSubmissions([]);
    }
    setLoading(false);
  }

  async function updateStatus(id, newStatus) {
    try {
      const res = await fetch("/api/ai-labels", {
        method: "PATCH",
        headers: { "Content-Type": "application/json", ...(await getAuthHeader()) },
        body: JSON.stringify({ id, status: newStatus, adminNotes: actionNote }),
      });
      if (res.ok) {
        setSubmissions(submissions.map(s =>
          s.id === id
            ? { ...s, status: newStatus, adminNote: actionNote, reviewedAt: new Date().toISOString() }
            : s
        ));
      }
    } catch (err) {
      console.error("Failed to update status", err);
    }
    setExpandedId(null);
    setActionNote("");
  }

  async function saveQuantity(id) {
    const quantity = parseInt(quantityDraft, 10);
    if (!quantity || quantity < 1) return;
    try {
      const res = await fetch("/api/ai-labels", {
        method: "PATCH",
        headers: { "Content-Type": "application/json", ...(await getAuthHeader()) },
        body: JSON.stringify({ id, quantity }),
      });
      if (res.ok) {
        setSubmissions(submissions.map(s => s.id === id ? { ...s, quantity } : s));
      }
    } catch (err) {
      console.error("Failed to update quantity", err);
    }
  }

  async function regenerateImage(id) {
    setRegeneratingId(id);
    try {
      const res = await fetch("/api/ai-labels", {
        method: "PATCH",
        headers: { "Content-Type": "application/json", ...(await getAuthHeader()) },
        body: JSON.stringify({ id, action: "regenerate_image" }),
      });
      const data = await res.json();
      if (res.ok) {
        setSubmissions(submissions.map(s => s.id === id ? { ...s, labelImageUrl: data.labelImageUrl } : s));
      } else {
        alert(data.error || "Failed to regenerate the label picture.");
      }
    } catch (err) {
      console.error("Failed to regenerate image", err);
      alert("Failed to regenerate the label picture.");
    }
    setRegeneratingId(null);
  }

  function deleteSubmission(id) {
    const updated = submissions.filter((s) => s.id !== id);
    setSubmissions(updated);
  }

  const filtered = filter === "all" ? submissions : submissions.filter((s) => s.status === (filter === "pending" ? "Pending" : filter === "approved" ? "Approved" : "Rejected"));

  const counts = {
    all: submissions.length,
    pending: submissions.filter((s) => s.status === "Pending").length,
    approved: submissions.filter((s) => s.status === "Approved").length,
    rejected: submissions.filter((s) => s.status === "Rejected").length,
  };

  const FILTERS = ["all", "pending", "approved", "rejected"];

  return (
    <AdminLayout>
      <AdminPageHeader
        title="AI Label Submissions"
        subtitle="Review generated labels, set print quantities, and approve or reject"
      />

      <div style={{ display: "flex", gap: "8px", marginBottom: "24px", flexWrap: "wrap" }}>
        {FILTERS.map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            style={{
              padding: "6px 16px",
              borderRadius: "4px",
              border: `1px solid ${filter === f ? "var(--accent)" : "var(--border-soft)"}`,
              background: filter === f ? "rgba(184,155,94,0.12)" : "var(--surface)",
              color: filter === f ? "var(--accent)" : "var(--text-muted)",
              fontSize: "16px",
              fontWeight: 600,
              cursor: "pointer",
              textTransform: "capitalize",
              letterSpacing: "0.05em",
              transition: "all 0.15s",
            }}
          >
            {f} {counts[f] > 0 && <span style={{ opacity: 0.7 }}>({counts[f]})</span>}
          </button>
        ))}
      </div>

      {loading ? (
        <p style={{ color: "var(--text-muted)", fontSize: "20px" }}>Loading…</p>
      ) : filtered.length === 0 ? (
        <div style={{
          textAlign: "center", padding: "64px 24px",
          border: "1px dashed var(--border-soft)", borderRadius: "6px",
          color: "var(--text-muted)",
        }}>
          <div style={{ fontSize: "clamp(26px, 4vw, 48px)", marginBottom: "16px" }}>🏷️</div>
          <p style={{ fontWeight: 600, marginBottom: "4px", color: "var(--text-main)" }}>
            No {filter === "all" ? "" : filter} submissions yet
          </p>
          <p style={{ fontSize: "16px" }}>
            Customers submit label copy from the{" "}
            <a href="/ai-label-studio" target="_blank" style={{ color: "var(--accent)" }}>
              AI Label Studio
            </a>
            .
          </p>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {filtered.map((s) => (
            <div
              key={s.id}
              style={{
                background: "var(--surface)",
                border: `1px solid ${expandedId === s.id ? "var(--accent)" : "var(--border-soft)"}`,
                borderRadius: "6px",
                overflow: "hidden",
                transition: "border-color 0.2s",
              }}
            >
              <div
                onClick={() => {
                  const opening = expandedId !== s.id;
                  setExpandedId(opening ? s.id : null);
                  setQuantityDraft(opening ? (s.quantity ?? s.eventCans ?? "") : "");
                }}
                className="responsive-grid-auto-4"
                style={{
                  gap: "16px",
                  alignItems: "center",
                  padding: "16px 20px",
                  cursor: "pointer",
                }}
              >
                <div style={{ minWidth: 0 }}>
                  <p style={{
                    fontSize: "20px",
                    color: "var(--text-main)",
                    fontStyle: "italic",
                    marginBottom: "4px",
                    overflow: "hidden",
                    textOverflow: "ellipsis",
                    whiteSpace: "nowrap",
                  }}>
                    "{s.labelText}"
                  </p>
                  <p style={{ fontSize: "14px", color: "var(--text-muted)" }}>
                    <strong style={{ color: "var(--text-main)" }}>{s.customerName}</strong>
                    {" · "}{s.eventType}{" · "}{s.tone} tone
                    {s.quantity ? ` · ${s.quantity} labels` : ""}
                    {" · "}{new Date(s.submittedAt).toLocaleDateString("en-CA", { month: "short", day: "numeric", year: "numeric" })}
                  </p>
                </div>

                <span className="hide-on-mobile" style={{ fontSize: "14px", color: "var(--text-muted)", flexShrink: 0 }}>
                  {s.id}
                </span>

                <StatusBadge status={s.status} />

                <span style={{ color: "var(--text-muted)", fontSize: "16px", flexShrink: 0 }}>
                  {expandedId === s.id ? "▲" : "▼"}
                </span>
              </div>

              {expandedId === s.id && (
                <div style={{
                  borderTop: "1px solid var(--border-soft)",
                  padding: "20px 20px 20px",
                  background: "var(--bg-soft)",
                }}>
                  <div style={{
                    padding: "20px 24px", marginBottom: "16px",
                    background: "#111", border: "1px solid var(--border-soft)",
                    borderRadius: "4px", textAlign: "center",
                  }}>
                    <p style={{ fontSize: "14px", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--text-muted)", marginBottom: "12px" }}>
                      Label Preview
                    </p>

                    {s.labelImageUrl ? (
                      <img
                        src={s.labelImageUrl}
                        alt={`Label: ${s.labelText}`}
                        style={{ width: "220px", height: "220px", borderRadius: "50%", margin: "0 auto", display: "block" }}
                      />
                    ) : (
                      <div>
                        <p style={{ fontSize: "24px", color: "#F7F3EA", fontStyle: "italic", lineHeight: "140%" }}>
                          {s.labelText}
                        </p>
                        <div style={{ width: "40px", height: "1px", background: "var(--accent)", margin: "12px auto 0" }} />
                        <p style={{ fontSize: "14px", letterSpacing: "0.15em", textTransform: "uppercase", color: "#77736B", marginTop: "8px" }}>
                          LÄYRD · Calgary
                        </p>
                        <p style={{ fontSize: "13px", color: "#f87171", marginTop: "12px" }}>
                          No label picture was generated yet.
                        </p>
                      </div>
                    )}

                    <div style={{ display: "flex", gap: "10px", justifyContent: "center", marginTop: "16px", flexWrap: "wrap" }}>
                      {s.labelImageUrl && (
                        <button
                          onClick={() => downloadImage(s.labelImageUrl, `${s.id}-label.png`)}
                          style={{
                            padding: "6px 16px", borderRadius: "4px",
                            background: "var(--surface)", border: "1px solid var(--border-soft)",
                            color: "var(--text-main)", fontSize: "14px", cursor: "pointer",
                          }}
                        >
                          ⬇ Download
                        </button>
                      )}
                      <button
                        onClick={() => regenerateImage(s.id)}
                        disabled={regeneratingId === s.id}
                        style={{
                          padding: "6px 16px", borderRadius: "4px",
                          background: "var(--surface)", border: "1px solid var(--border-soft)",
                          color: "var(--text-main)", fontSize: "14px", cursor: "pointer",
                        }}
                      >
                        {regeneratingId === s.id ? "Regenerating…" : "🔄 Regenerate Image"}
                      </button>
                    </div>
                  </div>

                  <div className="responsive-grid-2" style={{ gap: "20px", marginBottom: "16px" }}>
                    {[
                      ["Submission ID", s.id],
                      ["Event ID", s.eventId],
                      ["Event Type", s.eventType],
                      ["Event Date", s.eventDate],
                      ["Tone", s.tone],
                      ["Customer", s.customerName],
                      ["Submitted", new Date(s.submittedAt).toLocaleString("en-CA")],
                    ].map(([label, val]) => (
                      <div key={label} style={{
                        background: "var(--surface)", border: "1px solid var(--border-soft)",
                        borderRadius: "4px", padding: "10px 14px",
                      }}>
                        <p style={{ fontSize: "14px", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "3px" }}>{label}</p>
                        <p style={{ fontSize: "16px", color: "var(--text-main)", fontWeight: 500 }}>{val || "—"}</p>
                      </div>
                    ))}
                  </div>

                  <div style={{
                    marginBottom: "16px", padding: "14px 16px",
                    background: "var(--surface)", border: "1px solid var(--border-soft)",
                    borderRadius: "4px",
                  }}>
                    <label style={{ fontSize: "14px", color: "var(--text-muted)", display: "block", marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                      Labels needed to print
                      {s.eventCans > 0 && (
                        <span style={{ textTransform: "none", letterSpacing: "normal", fontWeight: 400 }}>
                          {" "}(event has {s.eventCans} cans total)
                        </span>
                      )}
                    </label>
                    <div style={{ display: "flex", gap: "10px" }}>
                      <input
                        type="number"
                        min="1"
                        value={quantityDraft}
                        onChange={(e) => setQuantityDraft(e.target.value)}
                        style={{
                          width: "120px", padding: "8px 12px",
                          background: "var(--surface)", border: "1px solid var(--border-soft)",
                          borderRadius: "4px", color: "var(--text-main)",
                          fontSize: "16px", outline: "none",
                        }}
                      />
                      <button
                        onClick={() => saveQuantity(s.id)}
                        style={{
                          padding: "8px 16px", borderRadius: "4px",
                          background: "rgba(184,155,94,0.12)", border: "1px solid var(--accent)",
                          color: "var(--accent)", fontSize: "14px", fontWeight: 600, cursor: "pointer",
                        }}
                      >
                        Save
                      </button>
                    </div>
                  </div>

                  {s.status === "Pending" && (
                    <div style={{ marginBottom: "14px" }}>
                      <label style={{ fontSize: "14px", color: "var(--text-muted)", display: "block", marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                        Note to customer (optional)
                      </label>
                      <input
                        value={actionNote}
                        onChange={(e) => setActionNote(e.target.value)}
                        placeholder="e.g. Approved! We'll print this on your cans."
                        style={{
                          width: "100%", padding: "8px 12px",
                          background: "var(--surface)", border: "1px solid var(--border-soft)",
                          borderRadius: "4px", color: "var(--text-main)",
                          fontSize: "16px", outline: "none", boxSizing: "border-box",
                        }}
                      />
                    </div>
                  )}

                  {s.status !== "Pending" && s.adminNote && (
                    <div style={{
                      marginBottom: "14px", padding: "10px 14px",
                      background: "var(--surface)", border: "1px solid var(--border-soft)",
                      borderRadius: "4px", fontSize: "16px", color: "var(--text-muted)",
                    }}>
                      <strong style={{ color: "var(--text-main)" }}>Admin note: </strong>{s.adminNote}
                    </div>
                  )}

                  <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                    {s.status === "Pending" && (
                      <>
                        <button
                          onClick={() => updateStatus(s.id, "Approved")}
                          style={{
                            padding: "8px 20px", borderRadius: "4px",
                            background: "rgba(74,222,128,0.12)", border: "1px solid rgba(74,222,128,0.4)",
                            color: "#4ade80", fontSize: "16px", fontWeight: 600,
                            cursor: "pointer", transition: "background 0.15s",
                          }}
                          onMouseEnter={(e) => e.currentTarget.style.background = "rgba(74,222,128,0.22)"}
                          onMouseLeave={(e) => e.currentTarget.style.background = "rgba(74,222,128,0.12)"}
                        >
                          ✓ Approve Label
                        </button>
                        <button
                          onClick={() => updateStatus(s.id, "Rejected")}
                          style={{
                            padding: "8px 20px", borderRadius: "4px",
                            background: "rgba(248,113,113,0.10)", border: "1px solid rgba(248,113,113,0.35)",
                            color: "#f87171", fontSize: "16px", fontWeight: 600,
                            cursor: "pointer", transition: "background 0.15s",
                          }}
                          onMouseEnter={(e) => e.currentTarget.style.background = "rgba(248,113,113,0.20)"}
                          onMouseLeave={(e) => e.currentTarget.style.background = "rgba(248,113,113,0.10)"}
                        >
                          ✕ Reject Label
                        </button>
                      </>
                    )}
                    {s.status !== "Pending" && (
                      <button
                        onClick={() => updateStatus(s.id, "Pending")}
                        style={{
                          padding: "8px 16px", borderRadius: "4px",
                          background: "var(--surface)", border: "1px solid var(--border-soft)",
                          color: "var(--text-muted)", fontSize: "16px",
                          cursor: "pointer", }}
                      >
                        ↩ Reset to Pending
                      </button>
                    )}
                    <button
                      onClick={() => deleteSubmission(s.id)}
                      style={{
                        padding: "8px 14px", borderRadius: "4px",
                        background: "transparent", border: "1px solid var(--border-soft)",
                        color: "var(--text-muted)", fontSize: "16px",
                        cursor: "pointer", marginLeft: "auto",
                      }}
                    >
                      Delete
                    </button>
                  </div>
                </div>
              )}
            </div>
          ))}
        </div>
      )}
    </AdminLayout>
  );
}