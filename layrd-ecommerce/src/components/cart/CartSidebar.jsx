...
export default function CartSidebar({ isOpen, onClose }) {
  const { items, subtotal, totalItems, deliveryFee, promoCode, removeItem, updateQuantity } =
    useCart();
      ...
      <div className={`cart-sidebar ${isOpen ? "open" : ""}`}>
        ...
        {items.length > 0 && (
          ...
          >
            ...
            <Link href="/cart" onClick={onClose} style={{ textAlign: "center", fontSize: "0.8rem", color: "var(--text-muted)" }}>
              View full cart
            </Link>
          </div>
        )}
