...
//
// TODO: Connect to Supabase — replace getBatches(), getCommittedItems(),
//       calculateStock() with server-side queries when ready.
// ─────────────────────────────────────────────
...
// WHY only constants imported directly now: BATCH_FLAVOURS/SIZES/CATEGORIES
// are plain arrays, safe client-side. The actual data-fetching functions
// (getBatches, createBatch, etc.) now go through /api/admin/inventory
// instead, since they use getSupabaseAdmin() (server-only, per TRD 7.1).
import { BATCH_FLAVOURS, BATCH_SIZES, BATCH_CATEGORIES } from "@/lib/admin-inventory";
// ── Helpers ──────────────────────────────────
...
export default function AdminInventoryPage() {
  ...
  async function handleSave(e) {
    ...
    if (editTarget) {
      await fetch(`/api/admin/inventory/${editTarget.id}`, {
        method: "PATCH",
  ...
  async function handleDelete() {
    await fetch(`/api/admin/inventory/${deleteTarget.id}`, { method: "DELETE" });
    await refreshData();
  ...
  if (loading) {
    return (
      <AdminLayout title="Inventory" subtitle="Loading...">
  ...
  return (
    <AdminLayout
      title="Inventory"
      subtitle={`${totalAvailable} units available across ${batches.length} batches`}
      actions={
        <button
    ...
    >
        ...
        <table className="table">
          ...
          <tbody>
            {batches.length === 0 ? (
              ...
              batches.map((b) => (
                <tr key={b.id}>
                  <td style={{ color: "var(--color-accent)", fontSize: "0.78rem", fontFamily: "monospace" }}>{b.id}</td>
      ...
      <ConfirmModal
        open={!!deleteTarget}
        title="Delete Batch"
        message={`Delete batch ${deleteTarget?.id} (${deleteTarget?.flavour} ${deleteTarget?.size}, ${deleteTarget?.qtyProduced} units)? This cannot be undone.`}
