...
- **Customers, business accounts, and admins are all Supabase Auth users** — there's a single identity system, differentiated by `profiles.role` (`customer` / `business` / `admin`). This replaced an older `admin_users` + cookie-session system that some docs (`layrd-ecommerce/PHASES.md`) still describe — that system is gone; trust the code, not `PHASES.md`, for current auth architecture.
